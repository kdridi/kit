...
abort "#{KIT_ROOT}/.git already exists!" unless Dir["#{KIT_ROOT}/.git/*"].empty?
...
if File.directory? "#{KIT_ROOT}"
  sudo "/bin/chmod", "g+rwx", *chmods unless chmods.empty?
  sudo "/usr/bin/chgrp", "admin", *chgrps unless chgrps.empty?
else
  sudo "/bin/mkdir #{KIT_ROOT}"
  sudo "/bin/chmod g+rwx #{KIT_ROOT}"
  # the group is set to wheel by default for some reason
  sudo "/usr/bin/chgrp admin #{KIT_ROOT}"
end
